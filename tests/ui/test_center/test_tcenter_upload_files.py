...
@allure.feature('Upload tab as Test Center Owner')
@pytest.mark.tc
@pytest.mark.daily
@pytest.mark.ui
@pytest.mark.usefixtures('logout_after_test')
class TestTCenterUploadFiles:  # pylint: disable=unused-argument, attribute-defined-outside-init
    ...
    @allure.title('C6488 Check the ability to add individual labor with valid data as a Test Center')
    def test_tc_adding_individual_labor(self, pre_test):
        self.spa_actions.add_individual_labor()
    ...
    def test_tc_upload_csv_file_with_selected_category_and_tc(self, pre_test, amount):
    ...
    def test_tc_upload_csv_file_without_selected_category_and_tc(self, pre_test):
        self.spa_actions.verify_upload_csv_file_without_selected_category_and_tcenter()

    @allure.title('C9121 C9126 Checking the ability to search entry by filters and clear filters on Upload Files table')
    def test_tc_search_entry_by_filters_on_upload_files_table(self, pre_test):
        self.spa_actions.verify_filters_on_uploads_tab()

    @allure.title('Checking pagination and search result value as Test Center on Upload tab')
    def test_tc_upload_tab_pagination_and_search_result_value(self, pre_test):
    ...
    def test_tc_entry_search_entry_by_filters_on_upload_files_table(self, pre_test):
        self.spa_actions.verify_filters_on_upload_files()

    @allure.title('Checking the ability to download csv sample')
    def test_tc_download_csv_sample(self, pre_test, clear_temp_folder):
