...
@allure.feature('Payment tab as Legislator')
@pytest.mark.legislator
@pytest.mark.daily
@pytest.mark.ui
@pytest.mark.usefixtures('logout_after_test')
class TestLegislatorPayment:  # pylint: disable=unused-argument, attribute-defined-outside-init
    ...
    @allure.title('C10345 Check the ability to add credits as a Legislator')
    def test_legislator_get_credits(self, pre_test):
        self.spa_payment_actions.verify_ability_to_add_credits()

    @allure.title('Check the ability to issue certificate as a Legislator')
    def test_legislator_issue_certificate(self, pre_test):
        self.spa_payment_actions.add_group()
        self.spa_payment_actions.spa_payment.verify_credits_counter(0)
        self.spa_payment_actions.spa_payment.verify_number_of_labors(0)
        self.spa_payment_actions.spa_payment.verify_total_amount(0)
        self.spa_payment_actions.verify_no_data_available()

    @allure.title('C10346 Check the ability to verify input fields on add credits as a Legislator')
    def test_legislator_get_verify_input_fields_on_add_credits(self, pre_test):
    ...
    @allure.title('C6951 C6952 C6953 Checking validate the issuing certificate and showing error message to agree '
                  'before made a duplicate payment as Legislator')
    def test_legislator_error_message_duplicate_payment(self, pre_test_without_creation_entities, amount=1):
        self.auth_api_actions.get_token()
        self.admin_api_actions.put_permissions(self.auth_api_actions.token, tcenter=False)
        self.login_actions.log_in_to_spa_with_permanent_email(tcenter=False)
        self.spa_payment_actions.verify_error_message_duplicate_payment(amount)

    @allure.title('Check number of labors and total amount counters on payment tab as a Legislator')
    def test_legislator_number_of_labors_and_total_amount_counters(self, pre_test_with_tcenter, amount=1):
    ...
    @allure.title('Check the ability to download certificate as a Legislator')
    ...
    @allure.title('Check Payment Confirmation info and Transaction Information as a Legislator')
    def test_legislator_payment_info(self, pre_test_with_tcenter):
        self.spa_payment_actions.verify_payment_information()

    @allure.title('Check the ability to download invoice after passing get credits flow as a Legislator')
    def test_legislator_invoice_after_passing_get_credits_flow(self, pre_test_with_tcenter, clear_temp_folder):
